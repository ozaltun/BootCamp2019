...
capture log close
log using "$output/Log_Country_counts_HS6-exports", replace text

*Bring in import data
use "$data/exports_tf_match.dta", clear
drop if substr(hs,1,2)=="27"

*Limit to sample and exporting firms
...
drop hs
rename hs6 hs
collapse (sum) evalue (mean) lvap emp sales, by(firmid hs country type)
...
/* basically, we want 7 samples: and for each of the 7 samples, we compute a mean for the 3 variables of interest:
(i) country per hs, mean within firm, (ii) country per hs, median within firm, (iii) country per hs, max within firm

sample 1: all firms
sample 2: 49-51 percentile of variable (i) across firms
sample 3: 49-51 percentile of variable (ii) across firms
sample 4: 49-51 percentile of variable (iii) across firms
sample 5: 94-96 percentile of variable (i) across firms
sample 6: 94-96 percentile of variable (ii) across firms
sample 7: 94-96 percentile of variable (iii) across firms

each sample is given by `var'r`val' == 1 in the notation below. 
`var' denotes variables (i)-(iii), and `val' denotes either 49-51 range (50), or 94-96 range (95)
*/
...
/*  ONLINE APPENDIX TABLE C.14  */ 
****************************************
capture log close   
log using "$output/Firm_Export_Stats_HS6.txt", replace text

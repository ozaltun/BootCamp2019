...
fprintf(fid, ' Entrants & %.3f & %.3f & %.3f & %.3f \\\\ \n',sum(dom_sourcing_after(entrants_china_id) .* m.weights_prod(entrants_china_id)) / sum(dom_sourcing(entrants_china_id) .* m.weights_prod(entrants_china_id)),sum(imp_other_after(entrants_china_id) .*m.weights_prod(entrants_china_id) ) / sum(imp_other(entrants_china_id) .* m.weights_prod(entrants_china_id)) ,sum(imp_china_after(entrants_china_id) .*m.weights_prod(entrants_china_id) ) / sum(imp_china(entrants_china_id) .* m.weights_prod(entrants_china_id)) , sum(m.weights_prod(entrants_china_id)))
fprintf(fid, ' Exiters &  %.3f & %.3f & %.3f & %.3f \\\\ \n',sum(dom_sourcing_after(exit_china_id) .* m.weights_prod(exit_china_id)) / sum(dom_sourcing(exit_china_id) .*m.weights_prod(exit_china_id)), sum(imp_other_after(exit_china_id) .*m.weights_prod(exit_china_id)) / sum(imp_other(exit_china_id) .* m.weights_prod(exit_china_id)) , sum(imp_china_after(exit_china_id) .*m.weights_prod(exit_china_id)) / sum(imp_china(exit_china_id) .* m.weights_prod(exit_china_id)) , sum(m.weights_prod(exit_china_id)))
fprintf(fid, ' Continuers &  %.3f & %.3f & %.3f & %.3f \\\\ \n',sum(dom_sourcing_after(cont_china_id).*m.weights_prod(cont_china_id)) / sum(dom_sourcing(cont_china_id).*m.weights_prod(cont_china_id)), sum(imp_other_after(cont_china_id).*m.weights_prod(cont_china_id)) / sum(imp_other(cont_china_id) .*m.weights_prod(cont_china_id)),sum(imp_china_after(cont_china_id).*m.weights_prod(cont_china_id)) / sum(imp_china(cont_china_id) .*m.weights_prod(cont_china_id)),sum(m.weights_prod(cont_china_id)))

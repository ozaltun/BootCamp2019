function [minimum,fval] = anneal(loss, parent, options,LB,UB,add_input)
...
% FELIX: ADDED LB AND UB , add_input AS INPUT ARGUMENTS. 

def = struct(...
        'CoolSched',@(T) (.8*T),...
        'Generator',@(x) ( min(max(x+(randperm(length(x))==length(x))*randn/100,LB),UB)),...
        'InitTemp',1,...
        'MaxConsRej',1000,...
        'MaxSuccess',20,...
        'MaxTries',300,...
...
current_all = [];
loss_all = [];
save simul_anneal_it current_all loss_all
...
consec = 0;
T = Tinit;
initenergy = loss(parent,add_input);
...
while ~finished;
    ...
    if itry >= max_try || success >= max_success;
        if T < minT || consec >= max_consec_rejections;
            ...
            if report==2, % output
                ...
                load simul_anneal_it
                current_all = [current_all;current];
                loss_all = [loss_all ; newenergy];
                save simul_anneal_it current_all loss_all
            end
            total = total + itry;
            itry = 1;
    ...
    newenergy = loss(newparam,add_input);

...
my_directory = pwd;
my_temp_num  = findstr(my_directory,'sample_sn');
my_new_seed_addition = str2num(my_directory(my_temp_num+9:end));   %sample_sn has 8 characters and then add 1 to start with first number
%my_new_seed_addition = 1;

% Set seeds
rand_seed                        = 6 + (my_new_seed_addition - 1);
randn_seed                       = 500 + (my_new_seed_addition - 1);
...
%my_directory                     = pwd;   
cd %GO TO DIRECTORY WHERE BS DATA IS STORED
% Go to boot directory for data.

filename1 = ['emp2_data_for_matlab_fe_2007_bs',num2str(my_new_seed_addition),'.out'];
filename2 = ['parameters_2007_bs',num2str(my_new_seed_addition),'.out' ];

%filename1                        = 'emp2_data_for_matlab_fe_2007_bs1.out';
country_data                     = importdata(filename1,',',1);
%filename2                        = 'parameters_2007_bs1.out';
add_data                         = importdata(filename2,',',1);         
...
m.rule_of_law                    = country_data.data(:,8);
m.rpshare                        = country_data.data(:,9);
m.export_cost                    = country_data.data(:,10);

m.US_median_dom_input            = add_data.data(8) / 1000;          


china_import_share07              = add_data.data(20);
china_import_share97              = add_data.data(21);


m.agg_china_import_share_percent_increase_data  = ((china_import_share07  / china_import_share97 ) - 1) * 100;  % 2007 share of imports over 1997 share of imports  % LOAD FROM PARAMETERS FILE 
...
m.share_importers_sales0_25 = add_data.data(10); 
m.share_importers_sales25_50 = add_data.data(11);  
m.share_importers_sales50_75 = add_data.data(12); 
m.share_importers_sales75_100 = add_data.data(13);

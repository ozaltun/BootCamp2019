...
% Initial guess
fc_mean_guess                    = .1 ;   % NEW Sep 8 2016, FT
B_guess                          = .05;
fc_disp_guess                    = 1;
...
% NEW: Make individual fixed cost draws the same across countries (pick
% Canadian draws [coun) , September 8 2016, FT
m.fc_shock_randn   = repmat(m.fc_shock_randn(:,can_ctr_ind),1,m.N);
...
ub(3) = 6;       % Upper bound for the dispersion parameter of the the fixed cost distribution
% NEW Sep 8 2016
MSRUN = 10;     % Number of Multistarts
beta_guess_all(1,1:length(beta_guess)) = beta_guess';
Guess_LB = [0.1  ;0.001 ;  .5  ];
Guess_UB = [0.15 ;0.2   ;  1.5 ];
...
if MSRUN > 3
   beta_guess_all(4,:)   =     [0.126 ,  0.0319,  1.1441];
end
if MSRUN > 4
   beta_guess_all(5,:)   =      [0.125038,0.01,  1 ];
end

if MSRUN > 6 
   beta_guess_all(7,:)   =      [0.126 , 0.08,    1.2912  ] ;
end

if MSRUN > 7 
   beta_guess_all(8,:)   =      [0.126 , 0.011,    .6 ] ;
end
